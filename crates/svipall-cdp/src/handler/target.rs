...
impl Target {
    ...
    pub fn on_event(&mut self, event: CdpEventMessage) {
        ...
        match &params {
            ...
            CdpEvent::TargetAttachedToTarget(ev) => {
                // PATCH: a worker is a realm of its own, and nothing the document was given
                // reaches it — so a worker reporting the host's real core count next to a
                // document reporting the identity's is a contradiction a page reads in one line.
                ...
                if ev.waiting_for_debugger && crate::worker::is_worker(&ev.target_info.r#type) {
                        ...
                        .filter(|s| !s.is_empty())
                    {
                        ...
                        self.queued_events.push_back(TargetEvent::Request(Request {
                            method: evaluate.identifier(),
                            session_id: Some(ev.session_id.clone().into()),
                            params: serde_json::to_value(evaluate).unwrap(),
                        }));
                    }
                }
                    ...
                    return match poll {
                        None => {
                            if let Some(isolated_world_cmds) = self
                                .frame_manager
                                .ensure_isolated_world(crate::world::utility_world_name())
                            {
    ...
    pub(crate) fn page_init_commands(timeout: Duration) -> CommandChain {
        let attach = SetAutoAttachParams::builder()
            ...
            .build()
            .unwrap();
        // PATCH: upstream also enables the Performance and Log domains here. Nothing in this crate
        // consumes `Performance.metrics` or `Log.entryAdded`, so both were pure protocol chatter on
        // every page. Two fewer commands per target is two fewer things that can behave differently
