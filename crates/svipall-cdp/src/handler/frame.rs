...
// PATCH: upstream hard-codes these two strings. They name the automation library in a place a
// page-side detector can read, so they are generated per process instead. See `crate::world`.
...
impl FrameManager {
    ...
    pub fn on_frame_execution_context_created(&mut self, event: &EventExecutionContextCreated) {
            ...
            .and_then(|v| v["frameId"].as_str())
        {
            if let Some(frame) = self.frames.get_mut(frame_id) {
                ...
                } else if event.context.name == crate::world::utility_world_name()
    ...
    pub fn ensure_isolated_world(&mut self, world_name: &str) -> Option<CommandChain> {
        ...
        let cmd = AddScriptToEvaluateOnNewDocumentParams::builder()
            .source(format!(
                "//# sourceURL={}",
                crate::world::evaluation_script_url()
            ))
