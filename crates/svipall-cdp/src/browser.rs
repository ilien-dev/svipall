...
impl BrowserConfig {
    pub fn launch(&self) -> io::Result<Child> {
        ...
        // `--hide-scrollbars` used to ride along with both headless modes. It makes
        // `window.innerWidth` equal `documentElement.clientWidth` on a page that must scroll,
        // which on a desktop Chrome they never are — the scrollbar is the difference. See
        // PATCHES.md §6.
        match self.headless {
            HeadlessMode::False => (),
            HeadlessMode::True => {
                cmd.args(["--headless", "--mute-audio"]);
            }
            HeadlessMode::New => {
                cmd.args(["--headless=new", "--mute-audio"]);
            }
        }
