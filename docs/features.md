...
- **Integrity verdict on assessed pages**: `full`, `partial` (possible truncation) or `thin`
  ...
  and `mostly_boilerplate`. That last one only ever comes from a **crawl**, because only a caller that has
  seen the rest of the site can know it, and a single fetch does not guess. Every
...
fields accompany supported assessed page results, and this option adds their detailed evidence.
...
*Different domains, different faces. Within a session, cookies and identity stay together, and
...
- **Browser-like network fingerprint**: selected JA4, HTTP/2 SETTINGS, header-order and GREASE checks on
  ...
  gap is named as open in [`CHANGELOG.md`](../CHANGELOG.md).
...
- **Pools of exits, used properly**: `web_route` takes several proxies per domain, each with its
  ...
  retired exit **heals with time**. Pacing, strikes and latency are keyed by
  ...
  paths but are not a network-wide leak guarantee. `web_route check` flags a `socks5://`
...
- **Real browser interaction**: `web_snapshot` returns the page as roles, accessible names and short
  ...
  `browser_open` / `browser_do` keep a session alive across calls. Deterministic, with no vision model.
...
- **A site's own search box**: `web_site_search` can learn a reusable query-URL pattern and fetch
  later queries through it. Sites without a discoverable form or reusable URL can fail, and the fetch
...
- **Concurrency sized to your machine**: browsers cost far more than HTTP requests, and a laptop
  ...
  from core count and open browsers and never raised above what the config allows.
...
- **Secret references in action arguments**: values from `~/.svipall/secrets.env` are substituted
  ...
  but does not redact a site that echoes them in content, screenshots or other returned data.
