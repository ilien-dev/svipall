...
job routes that let you follow a long crawl without holding a connection open for it.
...
`svipall-mcp` mounts the same router when `rest_port` is set, on its own listener. It shares one
...
REST inherits the CLI's contract verbatim. In the CLI's own words: *"the exit code … never depends
on what a site said: a page that was blocked is a successful report of a block"*. A client that read
a wall as a `5xx` would sit in a retry loop against something that is never going to move.
...
Unknown body fields are ignored, which is what MCP clients already get. A typo'd field is therefore
silent. That is deliberate: it is the price of one behaviour across both protocols.
...
It lives in its own file beside `config.toml` because `Config` is not serialised: writing a
...
The server mints the id, and `POST /v1/crawl` does not accept a new one. `resume_or_start` does not
...
`DELETE` sets a durable flag in the row that survives a restart and, if the job is running here,
...
per page and trimmed by the housekeeper (a real cost for a consumer that is a progress bar), and what
...
It never hangs. An unknown id is a 404 before any stream is opened.
...
tier, eight Chrome pages, which is more than `capacity::concurrency` would ever grant one crawl.
...
That incident now has a mechanism behind it as well as a rule: `core::reputation` keeps what
each address has spent with each host, lets it decay with a half-life, and slows down before
declining. See [`docs/exits.md`](exits.md). `max_jobs` and the one-crawl-per-site rule still
matter, since they stop two jobs spending the same budget twice as fast. The budget is what bounds
the total.
...
solver database. Local models still answer, but a challenge none of them can answer has nowhere to go.
