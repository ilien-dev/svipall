...
Most of these are permanent and deliberate, but one is a build you have to ask for, and it says so.

- **No bundled proxies or IP rotation service.** You bring your own exit, and Svipall configures the declared
...
- **Browser-specific fingerprint defenses can conflict with emulation.** Brave is a recorded case:
  ...
  User-Agent claiming Chrome. That is why Brave, Vivaldi and Opera are sorted last among
  detected browsers. A build **two or more majors** behind the stable channel is flagged for the opposite
