...
## Candidate 1 confirmation addendum

This run uses the classification-aware automatic-routing candidate documented in README.md.
The original baseline preparation paragraphs above describe the prior frozen executable.
Both confirmation arms instead use this candidate executable. Accounting is seeded from
the completed paired baseline, not the earlier automatic-only experiment. The workload,
