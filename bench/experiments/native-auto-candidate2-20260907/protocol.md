# Native versus automatic: candidate 2 protocol
...
budgets, remove holds or reset cooldowns. Initial accounting comes from the final home of the preceding completed public workload,
including the three separately recorded same-document HTML captures after candidate 1. Those
captures are outside this score and their spend is carried forward. This is one physical exit
with an existing history.
...
The original baseline included the browser scratch-profile fix from e60e10b,
the local test-isolation fix and a measurement-only Rust entry point. This candidate includes
the changes recorded below and in its source manifest. The optimized executable uses target-cpu=native and
...
## Candidate 2 confirmation addendum

This run uses the candidate documented in README.md: expiring fingerprint-wall route evidence,
managed-challenge transitions and record-heading preservation, plus the concurrently developed
MCP/tool-surface and snapshot changes captured in the manifest and covered by current-tree QC.
Both confirmation arms use the same candidate executable. Accounting is seeded from the three
separately recorded HTML captures following the completed candidate-1 workload. The workload,
schedule, limits, measurement, content criteria and privacy constraints are unchanged.
Fresh arm profiles match the baseline starting design; existing shared accounting is retained.
Elapsed-time decay and prior exposure prevent a causal before/after success claim.
The proposed blanket early acceptance of short pages was rejected by a local delayed-rendering
control. No warm-wait duration, caller deadline or policy budget is changed for this candidate.
The intended confirmation has only the two scheduled 120-second pauses; any unplanned pause
must be recorded and its impact assessed before describing the run as uninterrupted.
