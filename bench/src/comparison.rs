...
pub async fn run(args: &[String]) -> anyhow::Result<usize> {
    ...
    for target in targets {
        ...
        for position in 1..=repeats {
            ...
            let delivered = status.is_some_and(|s| (200..400).contains(&s))
                && result["blocked_reason"].is_null()
                && !content.trim().is_empty()
                && expected;
            let outcome = if !valid_status {
                "error"
            } else if delivered {
                "delivered"
            } else if !result["blocked_reason"].is_null() {
                "wall"
            } else {
                "missing_content"
            };
            ...
            cells.push(
                json!({"target":target.name,"url":target.url,"position":position,
    ...
    println!(
        "{}",
        json!({"schema":1,"label":flag("--label"),"set":set.name(),
